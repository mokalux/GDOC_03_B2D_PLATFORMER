...
function ButtonMonster:setButton()
	...
	-- text
	if self.text then
		self.text = TextField.new(self.ttf, self.text, self.text)
		self.text:setAnchorPoint(0.5, 0.5)
		self.text:setScale(self.textscalexup, self.textscaleyup)
		self.text:setTextColor(self.textcolorup)
		self.text:setAlpha(self.textalphaup)
	...
	if self.text then self.sprite:addChild(self.text) end
	...
	if self.btns then -- navigatable buttons
		for k, v in ipairs(self.btns) do
			if v.isdisabled then -- button is isdisabled
				...
				if v.text ~= nil then v.text:setTextColor(v.textcolordisabled) end
			elseif v.selector == v:getParent().selector then -- button is focused (down state)
				if v.hover then -- can hover option
					...
					if v.text ~= nil then v.text:setTextColor(v.textcolordown) v.text:setScale(v.textscalexdown, v.textscaleydown) end
				end
				...
				if v.text ~= nil then v.text:setTextColor(v.textcolorup) v.text:setScale(v.textscalexup, v.textscaleyup) end
			end
			-- tool tip
--			if v.tooltiptext and not v.isdisabled then -- OPTION 1: hides tooltip when button is disabled
		...
		if self.isdisabled then -- button is disabled
			...
			if self.text ~= nil then self.text:setTextColor(self.textcolordisabled) end
		elseif self.ishovered then -- button is focused (down state)
			...
			if self.text ~= nil then self.text:setTextColor(self.textcolordown) self.text:setScale(self.textscalexdown, self.textscaleydown) end
		else -- button is not focused (up state)
			...
			if self.text ~= nil then self.text:setTextColor(self.textcolorup) self.text:setScale(self.textscalexup, self.textscaleyup) end
		end
...
function ButtonMonster:selectionSfx()
	...
	if not self.channel:isPlaying() then self.channel = self.sound:play() end
end
